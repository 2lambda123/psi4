...
def table_builder__ref_driver_type_fcae():
    ...
    if args.mode in ["details", "summary"]:
        legend_lines.append(
            f"""Double underline "{trans_cell['[pass]'][args.sphinx]}" or "{trans_cell['[fd]'][args.sphinx]}" is default algorithm type when type selector (e.g., |globals__cc_type|\ ) unspecified."""
        )
    else:
        legend_lines.append(
            f"""Double underline "{trans_cell['[pass]'][args.sphinx]}" is default algorithm type when type selector (e.g., |globals__cc_type|\ ) unspecified."""
        )
    ...
    if args.mode == "summary":
        ...
        lines.append(fline_fill(dashh, dashh, dashh, [dashh] * ncol))
        lines.append(
            fline_data(place, blank, place, [f"{spacer}{qcmoduleh[:-2]}\ ={module_caption} Capabilities"], span=ncol))
        lines.append(fline_fill(blank, blank, blank, [dashh] * ncol))
        lines.append(fline_data(place, blank, referenceh, refs, span=len(fcaes) * len(corl_types) * len(eghs)))
        lines.append(fline_fill(blank, blank, blank, [dashh] * ncol))
        lines.append(fline_data(nameh, blank, place, len(refs) * eghs, span=len(fcaes) * len(corl_types)))
